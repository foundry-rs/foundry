...
impl<'a> RemappingsProvider<'a> {
    ...
    fn get_remappings(&self, remappings: Vec<Remapping>) -> Result<Vec<Remapping>, Error> {
        let mut unique_remappings: HashMap<_, _> =
            remappings.into_iter().map(|r| (r.name, r.path)).collect();
        ...
        if remappings_file.is_file() {
            ...
            let remappings = remappings.map_err(|err| err.to_string())?;
            unique_remappings.extend(remappings.into_iter().map(|r| (r.name, r.path)));
        }
        ...
        if unique_remappings.is_empty() {
            // if no remappings found detect them via lib path look ups
            for remapping in
                self.lib_paths.iter().map(|lib| self.root.join(lib)).flat_map(Remapping::find_many)
            {
                if !unique_remappings.contains_key(&remapping.name) {
                    unique_remappings.insert(remapping.name, remapping.path);
                }
            }
        }

        Ok(unique_remappings.into_iter().map(|(name, path)| Remapping { name, path }).collect())
    }
}

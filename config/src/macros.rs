...
/// A macro to implement converters from a type to [`Config`] and [`figment::Figment`]
///
/// It's functionally the same as [`impl_figment_convert!`] but it doesn't require types to have a
/// root field.
///
/// # Example
///
/// Use `impl_figment_no_root_convert` on a type.
///
/// ```rust
/// use std::path::PathBuf;
/// use serde::Serialize;
/// use foundry_config::{Config, impl_figment_no_root_convert};
/// use foundry_config::figment::*;
/// use foundry_config::figment::error::Kind::InvalidType;
/// use foundry_config::figment::value::*;
/// #[derive(Default, Serialize)]
...
/// }
/// impl_figment_no_root_convert!(MyArgs);
///
/// impl Provider for MyArgs {
///     fn metadata(&self) -> Metadata {
///         Metadata::default()
...
/// let config: Config = From::from(&MyArgs::default());
///
///  // Use `impl_figment_no_root` on a type that has several nested `Provider` as fields.
///
/// #[derive(Default)]
/// struct Outer {
///     start: MyArgs,
...
///     third: MyArgs,
/// }
/// impl_figment_no_root_convert!(Outer, start, second, third);
///
/// let figment: Figment = From::from(&Outer::default());
/// let config: Config = From::from(&Outer::default());
/// ```
...
#[macro_export]
macro_rules! impl_figment_no_root_convert {
    ($name:ty) => {
        impl<'a> From<&'a $name> for $crate::figment::Figment {
            fn from(args: &'a $name) -> Self {
                $crate::Config::figment_with_root($crate::find_project_root_path().unwrap())

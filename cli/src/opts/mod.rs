...
impl MultiWallet {
    pub fn interactives(&self) -> Result<Option<Vec<LocalWallet>>> {
        Ok(if self.interactives != 0 {
            let mut wallets = vec![];
            for _ in 0..self.interactives {
                println!("Insert private key:");
                let private_key = rpassword::read_password()?;
                let private_key = private_key.strip_prefix("0x").unwrap_or(&private_key);
                wallets.push(LocalWallet::from_str(private_key)?);
            }
            Some(wallets)
        } else {
            None
    ...
    pub fn private_keys(&self) -> Result<Option<Vec<LocalWallet>>> {
        Ok(if let Some(private_keys) = &self.private_keys {
            let mut wallets = vec![];
    ...
    pub fn keystores(&self) -> Result<Option<Vec<LocalWallet>>> {
        Ok(if let Some(keystore_paths) = &self.keystore_paths {
            let mut wallets = vec![];
            if let Some(passwords) = &self.keystore_passwords {
                for (path, keystore_password) in keystore_paths.iter().zip(passwords) {
                    wallets.push(LocalWallet::decrypt_keystore(path, keystore_password)?);
                }
            } else {
                for path in keystore_paths.iter() {
                    let password = rpassword::read_password().unwrap();
                    wallets.push(LocalWallet::decrypt_keystore(path, password)?);
                }
            }
            Some(wallets)
        } else {

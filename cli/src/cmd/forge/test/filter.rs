...
impl fmt::Debug for Filter {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Filter")
            ...
            .field("match-test", &self.test_pattern.as_ref().map(|r| r.iter().map(|x| x.as_str()).join(",")))
            .field("no-match-test", &self.test_pattern_inverse.as_ref().map(|r| r.iter().map(|x| x.as_str()).join(",")))
            .field("match-contract", &self.contract_pattern.as_ref().map(|r| r.iter().map(|x| x.as_str()).join(",")))
            .field("no-match-contract", &self.contract_pattern_inverse.as_ref().map(|r| r.iter().map(|x| x.as_str()).join(",")))
...
impl fmt::Display for Filter {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(ref p) = self.contract_pattern {
            patterns.push(format!("\tmatch-contract: `{}`", p.iter().map(|x| x.as_str()).join(",")));
        }
        if let Some(ref p) = self.contract_pattern_inverse {
            patterns.push(format!("\tno-match-contract: `{}`", p.iter().map(|x| x.as_str()).join(",")));
        }

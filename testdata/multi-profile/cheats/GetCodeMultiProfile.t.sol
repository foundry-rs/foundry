...
contract GetCodeMultiProfileTest is Test {
    function testGetCodeByProfileReturnsCorrectBytecode() public {
        ...
        bytes memory defaultCode = vm.getCode("multi-profile/ProfileCounter.sol:ProfileCounter:default");
        bytes memory optimizedCode = vm.getCode("multi-profile/ProfileCounter.sol:ProfileCounter:optimized");
        bytes memory unoptimizedCode = vm.getCode("multi-profile/ProfileCounter.sol:ProfileCounter:unoptimized");
    ...
    function testGetCodeByContractNameAndProfile() public {
        ...
        bytes memory optimizedCode = vm.getCode("ProfileCounter:optimized");
        bytes memory unoptimizedCode = vm.getCode("ProfileCounter:unoptimized");
    ...
    function testGetCodeProfileNotFound() public {
        ...
        vm.getCode("ProfileCounter:nonexistent_profile");
    }

    function testGetCodeProfileWithFullPath() public {
        // Test with full path format
        bytes memory code1 = vm.getCode("multi-profile/ProfileCounter.sol:ProfileCounter:optimized");
        bytes memory code2 = vm.getCode("ProfileCounter:optimized");
    ...
    function testDeployCodeWithProfile() public {
        ...
        address unoptimizedAddr = vm.deployCode("ProfileCounter:unoptimized");
        assertGt(unoptimizedAddr.code.length, 0, "should deploy unoptimized contract");

        // Deploy the optimized version
        address optimizedAddr = vm.deployCode("ProfileCounter:optimized");
    ...
    function testGetDeployedCodeWithProfile() public {
        ...
        bytes memory optimizedDeployed = vm.getDeployedCode("ProfileCounter:optimized");
        bytes memory unoptimizedDeployed = vm.getDeployedCode("ProfileCounter:unoptimized");

...
contract ParseTomlTest is Test {
    ...
    function test_parseTomlDefaults() public {
        string memory missing = ".missing";
        address defaultAddress = address(0x1234);
        bytes32 defaultBytes32 = keccak256("default");

        assertTrue(vm.parseTomlBool("", missing, true));
        assertEq(vm.parseTomlUint("", missing, 42), 42);
        assertEq(vm.parseTomlInt("", missing, -42), -42);
        assertEq(vm.parseTomlAddress("", missing, defaultAddress), defaultAddress);
        assertEq(vm.parseTomlString("", missing, "default"), "default");
        assertEq(vm.parseTomlBytes("", missing, hex"1234"), hex"1234");
        assertEq(vm.parseTomlBytes32("", missing, defaultBytes32), defaultBytes32);
        ...
        assertEq(abi.encode(vm.parseTomlBoolArray("", missing, defaultBools)), abi.encode(defaultBools));
        assertEq(abi.encode(vm.parseTomlUintArray("", missing, defaultUints)), abi.encode(defaultUints));
        assertEq(abi.encode(vm.parseTomlIntArray("", missing, defaultInts)), abi.encode(defaultInts));
        assertEq(abi.encode(vm.parseTomlAddressArray("", missing, defaultAddresses)), abi.encode(defaultAddresses));
        assertEq(abi.encode(vm.parseTomlStringArray("", missing, defaultStrings)), abi.encode(defaultStrings));
        assertEq(abi.encode(vm.parseTomlBytesArray("", missing, defaultBytes)), abi.encode(defaultBytes));
        assertEq(abi.encode(vm.parseTomlBytes32Array("", missing, defaultBytes32s)), abi.encode(defaultBytes32s));
        assertEq(abi.encode(vm.parseTomlUintArray("", missing, emptyUints)), abi.encode(emptyUints));

        assertEq(vm.parseTomlUint("value = 7", ".value", 42), 7);

        vm._expectCheatcodeRevert("expected uint256, found JSON object");
        vm.parseTomlUint("value = { nested = true }", ".value", 42);

        vm._expectCheatcodeRevert('path ".values[*]" must return exactly one JSON value');
        vm.parseTomlUint("values = [1, 2]", ".values[*]", 42);
    }

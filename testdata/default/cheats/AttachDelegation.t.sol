...
contract AttachDelegationTest is DSTest {
    ...
    function testAttachDelegation(uint64 nonce) public {
        SimpleDelegateContract implementation = new SimpleDelegateContract();
        ERC20 token = new ERC20(alice);

        (uint8 v, bytes32 r, bytes32 s) = vm.signDelegation(address(implementation), nonce, alice_pk);
        ...
        vm.broadcast(bob_pk);
        vm.attachDelegation(address(implementation), nonce, v, r, s);
        // TODO - how do we specify that the target of this tx is alice's EOA
        // fails if msg.sender is not implementation address (minter)

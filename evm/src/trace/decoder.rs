            ...
            if let Some(precompile_fn) = self.precompiles.get(&node.trace.address) {
                ...
                if bytes.len() >= 4 {
                    if let Some(funcs) = self.functions.get(&bytes[0..4]) {
                        ...
                        node.trace.data = RawOrDecodedCall::Decoded("create2".to_string(), String::new(), vec![]);
                    ...
                    node.trace.data = RawOrDecodedCall::Decoded("fallback".to_string(), String::new(), Vec::new());

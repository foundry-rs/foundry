...
impl Backend {
    ...
    /// Ths will clean up already loaded accounts that would be initialized without the correct data from the fork
    ///
    ...
    /// So when the first fork is initialized we replace these accounts with the actual account as it exists on the fork.
    fn prepare_init_journal_state(&mut self) {
        let loaded_accounts = self.fork_init_journaled_state.state.
            iter().filter(|(addr, acc)| !acc.is_existing_precompile && acc.is_touched && !self.is_persistent(addr)).map(|(addr, _)|addr)
        ...
        for fork in self.inner.forks_iter_mut() {
            ...
            for loaded_account in loaded_accounts.iter().copied() {
                ...
                let init_account = journaled_state.state.get_mut(&loaded_account).expect("exists; qed");

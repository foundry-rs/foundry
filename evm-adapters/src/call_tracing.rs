...
impl CallTraceArena {
	pub fn push_trace(&mut self, entry: usize, mut new_trace: CallTrace) -> CallTrace {
		if new_trace.depth == 0 {
            ...
            self.push_trace(*self.arena[entry].children.last().expect("Disconnected trace"), new_trace)
        }
	}
	...
	pub fn inner_number_of_logs(&self, node_idx: usize) -> usize {
		self.arena[node_idx].children.iter().fold(0, |accum, idx| {
			accum + self.arena[*idx].trace.prelogs.len() + self.arena[*idx].trace.logs.len() + self.inner_number_of_logs(*idx)
	...
	pub fn print_logs_in_order(&self, node_idx: usize) {
		self.arena[node_idx].trace.prelogs.iter().for_each(|(raw, _loc)| println!("prelog {}", raw.topics[0]));
    ...
    pub fn pretty_print<'a, S: Clone, E: crate::Evm<S>>(
    	...
    	if trace.created {
    		if let Some((name, (_abi, _code))) =
	            contracts.iter().find(|(_key, (_abi, code))| diff_score(code, &evm.code(trace.addr)) < 0.05)
	        {
            ...
            contracts.iter().find(|(_key, (_abi, code))| diff_score(code, &evm.code(trace.addr)) < 0.05)
        {
            ...
            for (func_name, overloaded_funcs) in abi.functions.iter() {
                for func in overloaded_funcs.iter() {
                    if func.selector() == trace.data[0..4] {
                        let params = func.decode_input(&trace.data[4..]).expect("Bad func data decode");
                    	let strings = params.iter().map(|param| {
                        	format!("{:?}", param)
                        ...
                        decoded_output = Some(func.decode_output(&trace.output[..]).expect("Bad func output decode"));
                    }
                }
            }
            ...
            if children_idxs.len() == 0 {
            	...
            	children_idxs.iter().enumerate().for_each(|(i, child_idx)| {
	                ...
	                if i == children_idxs.len() - 1 && trace.logs.len() == 0 && decoded_output.is_none() {
	                    self.pretty_print(*child_idx, contracts, evm, left.to_string().replace("├─", "│").replace("└─", "  ") + "  └─ ");
	                } else {
	                    self.pretty_print(*child_idx, contracts, evm, left.to_string().replace("├─", "│").replace("└─", "  ") + "  ├─ ");
	                }
	            });
            ...
            if let Some(decoded) = decoded_output {
            	let strings = decoded.iter().map(|param| {
                	format!("{:?}", param)
                }).collect::<Vec<String>>().join(", ");
            	println!(
	            	"{}  └─ {} {}",
            ...
            children_idxs.iter().enumerate().for_each(|(i, child_idx)| {
                ...
                if i == children_idxs.len() - 1 && trace.logs.len() == 0 {
                    self.pretty_print(*child_idx, contracts, evm, left.to_string().replace("├─", "│").replace("└─", "  ") + " _└─ ");
                } else {
                    self.pretty_print(*child_idx, contracts, evm, left.to_string().replace("├─", "│").replace("└─", "  ") + " _├─ ");
                }
            });
...
// very simple fuzzy matching to account for immutables. Will fail for small contracts that are basically all immutable vars

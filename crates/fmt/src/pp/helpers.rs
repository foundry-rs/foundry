...
impl Printer {
    pub(crate) fn word_space(&mut self, w: impl Into<Cow<'static, str>>) {
        self.word(w);
        self.space();
    }
    ...
    pub(crate) fn hardbreak_if_not_bol(&mut self) {
    ...
    pub(crate) fn space_if_not_bol(&mut self) {
        if !self.is_bol_or_only_ind() {
            self.space();
        }
    }
    ...
    pub(crate) fn space_or_nbsp(&mut self, breaks: bool) {
        if breaks {
            self.space();
        } else {
            self.nbsp();
    ...
    pub(crate) fn word_nbsp(&mut self, w: impl Into<Cow<'static, str>>) {
        self.word(w);
        self.nbsp();
    }
}

...
impl<'sess> State<'sess, '_> {
    ...
    const fn print_docs(&self, docs: &'_ ast::DocComments<'_>) {
        ...
        match cmnt.style {
            ...
            CommentStyle::BlankLine => {
                ...
                let twice = match self.last_token() {
                    ...
                    Some(Token::Begin(_) | Token::End) => true,
                    _ => false,
                };
                if twice {
                    config.hardbreak(&mut self.s);
    ...
    fn print_trailing_comment_inner(
        ...
        if let Some((_, n)) = self.comments.peek_trailing(self.sm, span_pos, next_pos) {
            let config = config
                .unwrap_or_else(|| CommentConfig::skip_ws().mixed_no_break().mixed_prev_space());
            while printed <= n {

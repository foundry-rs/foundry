...
contract CheatcodeEnvironmentPending {
    ...
    function tuple() public returns (uint256) {
        (uint256 saved, uint256 ignored) = (block.number, advance()); //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
        return saved + ignored;
    }

    function binary() public returns (uint256) {
        return block.number + advance(); //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
    }

    function arguments() public returns (uint256) {
        return first(block.number, advance()); //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
    }

    function timestamp() public returns (uint256) {
        (uint256 saved, uint256 ignored) = (block.timestamp, advanceTime()); //~WARN: `block.timestamp` may be reused across a Foundry environment mutation; capture it with `vm.getBlockTimestamp()` instead
        return saved + ignored;
    }

...
contract CheatcodeEnvironment is ClockHelpers {
    ...
    function readsOnBothSides() public returns (uint256, uint256) {
        uint256 before_ = block.timestamp; //~WARN: `block.timestamp` may be reused across a Foundry environment mutation; capture it with `vm.getBlockTimestamp()` instead
        skip(20);
    ...
    function tupleAlias() public returns (uint256) {
        (uint256 saved, uint256 other) = (block.number, 7); //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
    ...
    function namedReturn() public returns (uint256 saved) {
        saved = block.timestamp; //~WARN: `block.timestamp` may be reused across a Foundry environment mutation; capture it with `vm.getBlockTimestamp()` instead
        clock.warp(200);
        return;
    }

    function implicitReturn() public returns (uint256 saved) {
        saved = block.number; //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
        clock.roll(200);
    }

    function helperTuple() internal view returns (uint256, uint256) {
        return (block.number, 7); //~WARN: `block.number` may be reused across a Foundry environment mutation; capture it with `vm.getBlockNumber()` instead
    }

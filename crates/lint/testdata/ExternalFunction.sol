...
contract ExternalFunction {
    ...
    function setStored(bytes memory data) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        stored = data;
    }

    function multiArrayConsumer(uint256[] memory xs, address[] memory ys) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        owner = ys[0];
        if (xs.length > 0) owner = ys[xs.length - 1];
    }

    function structConsumer(Item memory item) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        owner = item.who;
    }

    function nestedReferenceConsumer(string memory s, uint256 v) public returns (uint256) { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        bytes memory bs = bytes(s);
        return bs.length + v;
    }

    function calledOnlyExternally(bytes memory payload) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        stored = payload;
    }

    function localUnaryStillExternal(bytes memory data) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        uint256 i = 0;
        i++;
        stored = data;
    }
...
contract UnrelatedSameName {
    ...
    function isolatedSuperTarget(bytes memory data) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        buf = data;
    }
}
...
contract EscapingParams is WithGuard {
    ...
    function readsOnly(ItemLib.Item memory item) public { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        uint256 amt = item.amount;
        owner = item.who;
        stored = abi.encodePacked(amt);
    }

    function modifierWithoutArgs(bytes memory data) public checkValues { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        stored = data;
    }

    function returnsParam(bytes memory data) public returns (bytes memory) { //~NOTE: `public` function with `memory` reference parameters is not referenced internally
        return data;
    }
}

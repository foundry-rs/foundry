...
declare_forge_lint!(REQUIRE_REVERT_IN_LOOP, Severity::Low, "require-revert-in-loop");
    ...
    fn check_function(&mut self, ctx: &LintContext, gcx: Gcx<'gcx>, func: &'gcx Function<'gcx>) {
        for_each_loop_item(gcx, func, false, |item| {
            ...
            if let Some(expr) = reported {
                ctx.span_lint(&REQUIRE_REVERT_IN_LOOP, expr.span, |diag| {
                    diag.primary_message("`require` or `revert` inside a loop");
                });
            }
        });
    }

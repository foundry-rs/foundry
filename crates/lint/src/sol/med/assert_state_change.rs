    ...
    match &expr.kind {
        ...
        ExprKind::Call(callee, args, named_args) => {
            ...
            if let ExprKind::Member(base, method) = &callee.kind
                ...
                && is_dynamic_array_or_bytes(hir, base)
            ...
            if candidates.is_empty()
                ...
                && lvalue_is_state_var(hir, base)
                && let Some(recv_ty) = receiver_type(hir, base)
            {
                let lib_candidates =
...
fn is_dynamic_array_or_bytes(hir: &Hir<'_>, expr: &Expr<'_>) -> bool {
    match &expr.peel_parens().kind {
...
fn is_address_like<'hir>(hir: &Hir<'hir>, expr: &'hir Expr<'hir>) -> bool {
    match &expr.peel_parens().kind {
        ...
        ExprKind::Tuple(exprs) => {
            ...
            match (iter.next(), iter.next()) {
                (Some(inner), None) => is_address_like(hir, inner),
                _ => false,
            }
        }
        _ => false,

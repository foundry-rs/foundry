...
async fn test_beacon_api_get_blobs() {
    ...
    // Verify we have blob data from all transactions
    let blobs = body["data"].as_array().unwrap();
    assert_eq!(blobs.len(), 3, "Expected 3 blobs from 3 transactions");
    ...
    let body: serde_json::Value = response.json().await.unwrap();
    let filtered_blobs = body["data"].as_array().unwrap();
    assert_eq!(filtered_blobs.len(), 1, "Expected 1 blob when filtering by single versioned_hash");
    ...
    let body: serde_json::Value = response.json().await.unwrap();
    let filtered_blobs = body["data"].as_array().unwrap();
    assert_eq!(filtered_blobs.len(), 2, "Expected 2 blobs when filtering by two versioned_hashes");

...
use alloy_consensus::{
    BlobTransactionSidecar, BlobTransactionSidecarVariant, SidecarBuilder, SimpleCoder, Transaction,
};
use alloy_eips::{
    Typed2718,
...
async fn can_send_eip4844_transaction_fork() {
    ...
    let tx = TransactionRequest::default()
        .with_from(alice)
        .with_to(bob)
        .with_blob_sidecar(BlobTransactionSidecarVariant::Eip4844(sidecar.clone()));
...
async fn can_send_eip4844_transaction_with_eip7594_sidecar_format() {
    ...
    alloy_network::TransactionBuilder4844::set_blob_sidecar_7594(&mut tx, sidecar);
...
async fn can_mine_blobs_when_exceeds_max_blobs() {
    ...
    let sidecar: BlobTransactionSidecar = sidecar.build().unwrap();
    tx.set_blob_sidecar(BlobTransactionSidecarVariant::Eip4844(sidecar));
    tx.set_nonce(1);

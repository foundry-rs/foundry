...
async fn test_fee_history_ignores_stale_cache_after_reset() {
    ...
    let new_block = provider.get_block(BlockId::number(1)).await.unwrap().unwrap();
    let new_base_fee = new_block.header.base_fee_per_gas.unwrap() as u128;
    let new_history =
        api.fee_history(U256::from(1), BlockNumberOrTag::Number(1), vec![]).await.unwrap();

    assert_ne!(old_history.base_fee_per_gas[0], new_base_fee);
    assert_eq!(new_history.base_fee_per_gas[0], new_base_fee);
}

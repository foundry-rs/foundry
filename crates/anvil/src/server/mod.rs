...
use handler::{HttpEthRpcHandler, PubSubEthRpcHandler, HttpEngineRpcHandler, PubSubEngineRpcHandler};
...
/// Configures an [axum::Server] that handles [EthApi] related JSON-RPC calls via HTTP and WS
pub fn serve_engine(addr: SocketAddr, api: EngineApi, config: ServerConfig) -> AnvilServer {
    let http = HttpEngineRpcHandler::new(api.clone());
    let ws = PubSubEngineRpcHandler::new(api);
    anvil_server::serve_http_ws(addr, config, http, ws)
}
...
///
/// if setting up the ipc connection was unsuccessful
pub fn spawn_ipc_engine(api: EngineApi, path: impl Into<String>) -> JoinHandle<io::Result<()>> {
    try_spawn_ipc_engine(api, path).expect("failed to establish ipc connection")
}

/// Launches an ipc server at the given path in a new task
pub fn try_spawn_ipc_engine(
    api: EngineApi,
    path: impl Into<String>,
) -> io::Result<JoinHandle<io::Result<()>>> {
    let path = path.into();
    let handler = PubSubEngineRpcHandler::new(api);
    let ipc = IpcEndpoint::new(handler, path);
    let incoming = ipc.incoming()?;

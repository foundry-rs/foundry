...
use anvil_core::eth::{
    ...
    transaction::{
        PendingTransaction, TransactionInfo, TypedReceipt, TypedTransaction,
    },
};
use foundry_evm::{
    ...
    fn create_receipt(&self, cumulative_gas_used: &mut u64) -> TypedReceipt {
        ...
        *cumulative_gas_used = cumulative_gas_used.saturating_add(self.gas_used);
        ...
        match &self.transaction.pending_transaction.transaction.transaction {
            ...
            TypedTransaction::Deposit(_tx) => TypedReceipt::Deposit(op_alloy_consensus::OpDepositReceiptWithBloom {

...
pub(crate) fn build_foundry_receipt(
    ...
    let receipt =
        alloy_consensus::Receipt { status: Eip658Value::Eip658(status), cumulative_gas_used, logs }
            .with_bloom();
...
impl ReceiptBuilder for FoundryReceiptBuilder {
    ...
    fn build_receipt<E: Evm>(
        ...
        #[cfg(feature = "optimism")]
        assert_ne!(ctx.tx_type, FoundryTxType::Deposit, "deposit receipts are built separately");
        build_foundry_receipt(
            ctx.tx_type,
            ctx.result.is_success(),
            ctx.cumulative_gas_used,
            ctx.result.into_logs(),
            None,
        )
    }
}

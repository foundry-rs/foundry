...
pub enum MaybeOfflineForkedDatabase {
    Online(ForkedDatabase<AnyNetwork>),
    Offline(OfflineForkedDatabase),
}

impl MaybeOfflineForkedDatabase {
    pub fn online(db: ForkedDatabase<AnyNetwork>) -> Self {
        Self::Online(db)
    }
...
impl Database for MaybeOfflineForkedDatabase {
    type Error = foundry_evm::backend::DatabaseError;
...
impl revm::database::DatabaseRef for MaybeOfflineForkedDatabase {
    type Error = foundry_evm::backend::DatabaseError;
...
impl MaybeForkedDatabase for MaybeOfflineForkedDatabase {
    fn maybe_reset(&mut self, urls: Vec<String>, block_number: BlockId) -> Result<(), String> {
        match self {
            Self::Online(db) => db.maybe_reset(urls, block_number),
            Self::Offline(db) => db.maybe_reset(urls, block_number),
        }
    }

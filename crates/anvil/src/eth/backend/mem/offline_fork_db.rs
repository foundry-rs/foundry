...
pub struct OfflineForkedDatabase {
    inner: ForkedDatabase<AnyNetwork>,
}

impl OfflineForkedDatabase {
    pub fn new(inner: ForkedDatabase<AnyNetwork>) -> Self {
        Self { inner }
    }

    pub fn inner(&self) -> &ForkedDatabase<AnyNetwork> {
        &self.inner
    }

    pub fn inner_mut(&mut self) -> &mut ForkedDatabase<AnyNetwork> {
        &mut self.inner
    }

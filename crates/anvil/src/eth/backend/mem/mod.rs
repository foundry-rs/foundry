    ...
    N: Network<TxEnvelope = FoundryTxEnvelope, ReceiptEnvelope = FoundryReceiptEnvelope>,
{
    ...
    pub(crate) async fn apply_fork_transaction_replay(
        ...
        self.store_block(block_info, block_hash, |_storage| {
            #[cfg(feature = "monad")]
            if let Some(replay) = &mut monad_replay {
                replay.store_metadata(_storage, block_hash);
            }
        });
    ...
    fn store_block(
        ...
        block_hash: B256,
        store_metadata: impl FnOnce(&mut BlockchainStorage<N>),
    ) {

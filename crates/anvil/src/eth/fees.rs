        ...
        if let (Some(block), Some(receipts)) = (current_block, current_receipts) {
            ...
            let mut transactions: Vec<(u64, u64)> = receipts
                ...
                .map(|(i, receipt)| {
                    ...
                    {
                        Some(TypedTransaction::Enveloped(alloy_consensus::TxEnvelope::Legacy(t))) => {
                            U256::from(t.gas_price).saturating_sub(base_fee).to::<u64>()
                        }
                        Some(TypedTransaction::Enveloped(alloy_consensus::TxEnvelope::TaggedLegacy(t))) => {
                            U256::from(t.gas_price).saturating_sub(base_fee).to::<u64>()
                        }
                        Some(TypedTransaction::Enveloped(alloy_consensus::TxEnvelope::Eip2930(t))) => {
                            U256::from(t.gas_price).saturating_sub(base_fee).to::<u64>()
                        }
                        Some(TypedTransaction::Enveloped(alloy_consensus::TxEnvelope::Eip1559(t))) => {

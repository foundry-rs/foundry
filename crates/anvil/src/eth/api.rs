...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_setGCPercent` endpoint.
    pub fn debug_set_gc_percent(&self, _value: i32) -> Result<()> {
        node_info!("debug_setGCPercent");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugSetGcPercent(value) => {
                self.debug_set_gc_percent(value).to_rpc_result()
            }

...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_setHead` endpoint.
    pub fn debug_set_head(&self, _number: U64) -> Result<()> {
        node_info!("debug_setHead");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugSetHead(number) => self.debug_set_head(number).to_rpc_result(),

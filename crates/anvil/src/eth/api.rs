...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_getAccessibleState` endpoint.
    pub fn debug_get_accessible_state(&self, _from: BlockNumber, _to: BlockNumber) -> Result<()> {
        node_info!("debug_getAccessibleState");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugGetAccessibleState(from, to) => {
                self.debug_get_accessible_state(from, to).to_rpc_result()
            }

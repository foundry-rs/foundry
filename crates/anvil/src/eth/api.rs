...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_preimage` endpoint.
    pub fn debug_preimage(&self, _hash: B256) -> Result<()> {
        node_info!("debug_preimage");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugPreimage(hash) => self.debug_preimage(hash).to_rpc_result(),

...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_memStats` endpoint.
    pub fn debug_mem_stats(&self) -> Result<()> {
        node_info!("debug_memStats");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugMemStats(()) => self.debug_mem_stats().to_rpc_result(),

...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_printBlock` endpoint.
    pub fn debug_print_block(&self, _number: u64) -> Result<()> {
        node_info!("debug_printBlock");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugPrintBlock(number) => self.debug_print_block(number).to_rpc_result(),

...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_gcStats` endpoint.
    pub fn debug_gc_stats(&self) -> Result<()> {
        node_info!("debug_gcStats");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugGcStats(()) => self.debug_gc_stats().to_rpc_result(),

...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_standardTraceBadBlockToFile` endpoint.
    pub fn debug_standard_trace_bad_block_to_file(
        &self,
        _block: BlockNumber,
        _opts: Option<GethDebugTracingCallOptions>,
    ) -> Result<()> {
        node_info!("debug_standardTraceBadBlockToFile");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugStandardTraceBadBlockToFile(block, opts) => {
                self.debug_standard_trace_bad_block_to_file(block, opts).to_rpc_result()
            }

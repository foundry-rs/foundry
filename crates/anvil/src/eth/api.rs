...
impl<N: Network> EthApi<N> {
    ...
    /// Handles reth's no-op `debug_dumpBlock` endpoint.
    pub fn debug_dump_block(&self, _number: BlockId) -> Result<()> {
        node_info!("debug_dumpBlock");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugDumpBlock(number) => self.debug_dump_block(number).to_rpc_result(),

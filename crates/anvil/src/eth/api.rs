...
impl<N: Network> EthApi<N> {
    ...
    /// Flattens the entire key-value database into a single level.
    ///
    /// Handler for RPC call: `debug_chaindbCompact`.
    pub fn debug_chaindb_compact(&self) -> Result<()> {
        node_info!("debug_chaindbCompact");
        Ok(())
    }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::DebugChaindbCompact(()) => self.debug_chaindb_compact().to_rpc_result(),

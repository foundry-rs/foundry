...
pub enum EthRequest {
    ...
    /// reth's `debug_getAccessibleState` endpoint.
    #[serde(rename = "debug_getAccessibleState")]
    DebugGetAccessibleState(BlockNumber, BlockNumber),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_get_accessible_state() {
        let s = r#"{"method": "debug_getAccessibleState", "params": ["earliest", "latest"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugGetAccessibleState(_, _) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

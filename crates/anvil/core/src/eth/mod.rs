...
pub enum EthRequest {
    ...
    /// reth's `debug_printBlock` endpoint.
    #[serde(rename = "debug_printBlock", with = "sequence")]
    DebugPrintBlock(u64),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_print_block() {
        let s = r#"{"method": "debug_printBlock", "params": [0]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugPrintBlock(0) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

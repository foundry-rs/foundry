...
pub enum EthRequest {
    ...
    /// reth's `debug_dbAncients` endpoint.
    #[serde(rename = "debug_dbAncients", with = "empty_params")]
    DebugDbAncients(()),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_db_ancients() {
        let s = r#"{"method": "debug_dbAncients", "params": []}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugDbAncients(()) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

...
pub enum EthRequest {
    ...
    /// reth's `debug_setTrieFlushInterval` endpoint.
    #[serde(rename = "debug_setTrieFlushInterval", with = "sequence")]
    DebugSetTrieFlushInterval(String),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_set_trie_flush_interval() {
        let s = r#"{"method": "debug_setTrieFlushInterval", "params": ["1s"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugSetTrieFlushInterval(interval) => assert_eq!(interval, "1s"),
            req => panic!("unexpected request: {req:?}"),
        }
    }

...
pub enum EthRequest {
    ...
    /// reth's `debug_seedHash` endpoint.
    #[serde(rename = "debug_seedHash", deserialize_with = "deserialize_u64_seq")]
    DebugSeedHash(u64),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_seed_hash() {
        let s = r#"{"method": "debug_seedHash", "params": [1]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugSeedHash(number) => assert_eq!(number, 1),
            req => panic!("unexpected request: {req:?}"),
        }

        let s = r#"{"method": "debug_seedHash", "params": ["0x1"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugSeedHash(number) => assert_eq!(number, 1),
            req => panic!("unexpected request: {req:?}"),

...
pub enum EthRequest {
    ...
    /// reth's `debug_memStats` endpoint.
    #[serde(rename = "debug_memStats", with = "empty_params")]
    DebugMemStats(()),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_mem_stats() {
        let s = r#"{"method": "debug_memStats", "params": []}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugMemStats(()) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

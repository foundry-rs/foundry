...
pub enum EthRequest {
    ...
    /// reth's `debug_dumpBlock` endpoint.
    #[serde(rename = "debug_dumpBlock", with = "sequence")]
    DebugDumpBlock(BlockId),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_dump_block() {
        let s = r#"{"method": "debug_dumpBlock", "params": ["latest"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugDumpBlock(_) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

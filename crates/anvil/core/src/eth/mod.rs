...
pub enum EthRequest {
    ...
    /// reth's `debug_setGCPercent` endpoint.
    #[serde(rename = "debug_setGCPercent", with = "sequence")]
    DebugSetGcPercent(i32),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_set_gc_percent() {
        let s = r#"{"method": "debug_setGCPercent", "params": [100]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugSetGcPercent(100) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

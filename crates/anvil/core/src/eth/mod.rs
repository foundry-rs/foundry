...
pub enum EthRequest {
    ...
    /// reth's `debug_standardTraceBadBlockToFile` endpoint.
    #[serde(rename = "debug_standardTraceBadBlockToFile")]
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_standard_trace_bad_block_to_file() {
        let s = r#"{"method": "debug_standardTraceBadBlockToFile", "params": ["latest"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugStandardTraceBadBlockToFile(_, None) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

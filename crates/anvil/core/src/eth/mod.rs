...
pub enum EthRequest {
    ...
    /// reth's `debug_chaindbCompact` endpoint.
    #[serde(rename = "debug_chaindbCompact", with = "empty_params")]
    DebugChaindbCompact(()),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_chaindb_compact() {
        let s = r#"{"method": "debug_chaindbCompact", "params": []}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugChaindbCompact(()) => {}
            req => panic!("unexpected request: {req:?}"),
        }
    }

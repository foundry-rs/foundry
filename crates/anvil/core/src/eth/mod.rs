...
pub enum EthRequest {
    ...
    /// reth's `debug_setHead` endpoint.
    #[serde(rename = "debug_setHead", with = "sequence")]
    DebugSetHead(U64),
...
true}]}"#;
    ...
    #[test]
    fn test_serde_debug_set_head() {
        let s = r#"{"method": "debug_setHead", "params": ["0x0"]}"#;
        let value: serde_json::Value = serde_json::from_str(s).unwrap();
        match serde_json::from_value::<EthRequest>(value).unwrap() {
            EthRequest::DebugSetHead(number) => assert_eq!(number, U64::ZERO),
            req => panic!("unexpected request: {req:?}"),
        }
    }

...
impl MultiSolMacroGen {
    ...
    pub fn generate_bindings(&mut self, all_derives: bool) -> Result<()> {
        ...
        // Fix naming collisions where a contract's events enum shadows an external module
        self.fix_events_naming_collisions();
    ...
    /// Fixes naming collisions in generated bindings where a contract's events enum shadows
    /// an external contract/interface module with the same name.
    ///
    /// This happens when:
    /// - Contract `Foo` has events, generating `pub enum FooEvents` inside `mod Foo`
    /// - There's also an interface `FooEvents` generating `pub mod FooEvents`
    /// - References to `FooEvents::SomeType` inside `mod Foo` resolve to the local enum
    ///   instead of the external module
    ///
    /// The fix adds `super::` prefix to such references.
    fn fix_events_naming_collisions(&mut self) {
        // Collect all contract names that could cause collisions
        let contract_names: HashSet<String> =
            self.instances.iter().map(|i| i.name.clone()).collect();

        // For each contract, check if its events enum name collides with another contract
        for instance in &mut self.instances {
            let events_enum_name = format!("{}Events", instance.name);

            // If there's another contract with this name, we need to fix references
            if contract_names.contains(&events_enum_name)
                && let Some(tokens) = &mut instance.expansion
            {
                let code = tokens.to_string();

                // Replace references to the colliding name with super:: prefixed version.
                // We target specific patterns in type positions generated by alloy-sol-macro-expander.
                let fixed_code = Self::fix_type_references(&code, &events_enum_name);

                if fixed_code != code
                    && let Ok(fixed_tokens) = TokenStream::from_str(&fixed_code)
                {
                    *tokens = fixed_tokens;
                }
            }
    ...
    /// Fix type references to a colliding name by adding super:: prefix.
    fn fix_type_references(code: &str, colliding_name: &str) -> String {
        // Pattern: references like `CollisionName::SomeType` in type positions
        // that are NOT:
        // - The enum definition: `pub enum CollisionName`
        // - Inside the events enum definition block
        // - Already prefixed with super::
        //
        // We look for patterns like:
        // - `<CollisionName::` (start of a type constraint)
        // - `(CollisionName::` (tuple type or function param)
        // - `, CollisionName::` (in a list)
        // - `= CollisionName::` (type assignment)

        let patterns = [
            (format!("< {colliding_name} ::"), format!("< super :: {colliding_name} ::")),
            (format!("<{colliding_name} ::"), format!("< super :: {colliding_name} ::")),
            (format!("( {colliding_name} ::"), format!("( super :: {colliding_name} ::")),
            (format!("({colliding_name} ::"), format!("( super :: {colliding_name} ::")),
            (format!(", {colliding_name} ::"), format!(", super :: {colliding_name} ::")),

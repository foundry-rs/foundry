...
const BINDINGS_HEADER: &str = r#"#![allow(unused_imports, unused_attributes, clippy::all, rustdoc::all)]
...
"#
        ...
        if single_file {
            write_single_file(&lib_path, &self.instances).wrap_err("Failed to write lib.rs")?;
            return Ok(());
        }

        let mut lib_contents = BINDINGS_HEADER.to_string();
    ...
    pub fn write_to_module(
        ...
        if single_file {
            write_single_file(&mod_path, &self.instances).wrap_err("Failed to write mod.rs")?;
            return Ok(());
        }

        let mut mod_contents = BINDINGS_HEADER.to_string();
        for instance in &self.instances {
            let name = instance.name.to_snake_case();
...
fn write_single_file(path: &Path, instances: &[SolMacroGen]) -> Result<()> {
    let file = fs::create_file(path)?;
    let mut writer = BufWriter::new(file);
    writer.write_all(BINDINGS_HEADER.as_bytes())?;
    for instance in instances {
        writer.write_all(instance.expansion.as_ref().unwrap().as_bytes())?;
    }
    writer.flush()?;
    Ok(())
}

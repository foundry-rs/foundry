...
pub struct TestArgs {
    ...
    /// Consecutive worker-local runs without new coverage before corpus sync.
    #[arg(long, env = "FOUNDRY_INVARIANT_CORPUS_SYNC_PLATEAU_RUNS", value_name = "RUNS")]
    pub invariant_corpus_sync_plateau_runs: Option<u32>,

    /// Seconds without new coverage before corpus sync.
    #[arg(long, env = "FOUNDRY_INVARIANT_CORPUS_SYNC_PLATEAU_TIMEOUT", value_name = "SECONDS")]
    pub invariant_corpus_sync_plateau_timeout: Option<u64>,
...
impl Provider for TestArgs {
    ...
    fn data(&self) -> Result<Map<Profile, Dict>, figment::Error> {
        ...
        if let Some(plateau_runs) = self.invariant_corpus_sync_plateau_runs {
            invariant_corpus_sync_dict.insert("plateau_runs".to_string(), plateau_runs.into());
        }
        if let Some(plateau_timeout) = self.invariant_corpus_sync_plateau_timeout {
            invariant_corpus_sync_dict
                .insert("plateau_timeout".to_string(), plateau_timeout.into());
        }
...
mod tests {
    ...
    fn invariant_corpus_sync_args_merge_into_figment() {
        let args = TestArgs::parse_from([
            "foundry-cli",
            "--invariant-corpus-sync-plateau-runs",
            "9",
            "--invariant-corpus-sync-plateau-timeout",
            "12",
        ...
        let figment = figment::Figment::from(&args);
        assert_eq!(figment.extract_inner::<u32>("invariant.corpus_sync.plateau_runs").unwrap(), 9);
        assert_eq!(
            figment.extract_inner::<u64>("invariant.corpus_sync.plateau_timeout").unwrap(),
            12
        );

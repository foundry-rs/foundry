...
impl<'a> FunctionRunner<'a> {
    ...
    fn extract_test_backtrace(
        ...
        for addr in seen_addresses {
            if let std::collections::btree_map::Entry::Vacant(e) = all_contracts.entry(addr) {
                    ...
                    && !code.is_empty()
                {
                    ...
                    // Look in the complete known_contracts list which includes all compiled
                    // contracts
                    if let Some((id, _)) =
                        self.cr.mcr.known_contracts.find_by_deployed_code(code.bytes_ref().as_ref())
                    {
                        // Found a match!
                        if let Some(contract_data) = self.cr.mcr.contracts.get(id) {
                            e.insert((id.name.clone(), contract_data.abi.clone()));
                        } else {
                            // At least use the name from the artifact ID
                            e.insert((id.name.clone(), Default::default()));
                        }
                    }
                }
            }
        ...
        for node in traces.arena.nodes() {
            ...
            if !all_contracts.contains_key(&addr) {
                    ...
                    if let Some(contract_part) = label.split("::").next() {
                        ...
                        for (artifact_id, contract_data) in &self.cr.mcr.contracts {
                            if artifact_id.name == contract_part {
                                all_contracts.insert(
                                    addr,
                                    (contract_part.to_string(), contract_data.abi.clone()),
                                );
                                found = true;
                                break;
                            }
                        ...
                        if !found {
                            ...
                            all_contracts
                                .insert(addr, (contract_part.to_string(), Default::default()));
                        }
                    }
                }
                ...
                if !all_contracts.contains_key(&addr)
                    && let Ok(Some(account)) = self.executor.backend().basic_ref(addr)
                    && let Some(code) = &account.code
                    && !code.is_empty()
                {
                    ...
                    if let Some((id, _)) =
                        self.cr.mcr.known_contracts.find_by_deployed_code(code.bytes_ref().as_ref())
                        && let Some(contract_data) = self.cr.mcr.contracts.get(id)
                    {
        ...
        for (addr, (name, _abi)) in &all_contracts {
            ...
            for (artifact_id, source_maps) in &self.cr.mcr.source_maps {
                ...
                if artifact_id.name == *name || artifact_id.name.ends_with(&format!(":{name}")) {
            ...
            if !found {
                ...
                for id in self.cr.mcr.source_maps.keys() {
        ...
        for artifact_id in self.cr.mcr.deployed_bytecodes.keys() {
            tracing::info!("Available artifact: {}", artifact_id.name);
        }

        // Log all contracts we're looking for
        for (addr, (name, _)) in &all_contracts {
            tracing::info!("Contract in all_contracts: {} at address {}", name, addr);
        }

        for (addr, (name, _)) in &all_contracts {
            // Find the deployed bytecode for this contract
            tracing::info!("Looking for deployed bytecode for {} at address {}", name, addr);

            // Extract just the contract name without path prefixes
            let contract_only =
                if let Some(idx) = name.rfind(':') { &name[idx + 1..] } else { name };
        ...
        // Log what we found
        tracing::info!("Found source maps for {} addresses", address_to_source_map.len());
        for addr in address_to_source_map.keys() {
            tracing::info!("Have source map for address: {}", addr);
        }

        tracing::info!("Found deployed bytecodes for {} addresses", deployed_bytecodes.len());
    ...
    fn run_unit_test(mut self, func: &Function) -> TestResult {
        ...
        if !success
            && self.tcfg.verbosity >= 3
            && let Some(ref traces) = raw_call_result.traces
        {

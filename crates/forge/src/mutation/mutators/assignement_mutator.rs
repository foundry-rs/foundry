...
use crate::mutation::mutators::{Mutator, MutationContext};
...
impl Mutator for AssignmentMutator {
    fn generate_mutants(&self, context: &MutationContext<'_>) -> Result<Vec<Mutant>> {
        ...
        let assign_type = determinate_type(context.expr.unwrap()).context("AssignementMutator: unexpected expression kind");
...
fn determinate_type(expr: &Expr<'_>) -> Result<AssignVarTypes> {
    match &expr.kind {
        ExprKind::Lit(kind, _) => {
            Ok(AssignVarTypes::Literal(kind.kind.clone()))
        },
        ExprKind::Ident(val) => {
            Ok(AssignVarTypes::Identifier(val.to_string()))
        },
        _ => {

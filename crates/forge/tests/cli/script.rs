...
//         cmd.arg("script").arg(script).args(["--fork-url", rpc.as_str(),
// "-vvvvv"]).assert_success();     }
// );
...
//     let node_config =
// NodeConfig::test().with_eth_rpc_url(Some(rpc::next_http_archive_rpc_url()));     let (_api,
// handle) = spawn(node_config).await;     let dev = handle.dev_accounts().next().unwrap();
...
//     let node_config =
// NodeConfig::test().with_eth_rpc_url(Some(rpc::next_http_archive_rpc_url()));     let (_api,
// handle) = spawn(node_config).await;     let private_key =
...
// ).expect("Could not decode create2 deployer init_code").into();     api.anvil_set_code(addr,
// code).await.unwrap();
...
//     let re =
// Regex::new(r#"((timestamp":).[0-9]*)|((blockHash":).*)|((blockNumber":).*)|((cumulativeGasUsed":
// ).*)|((effectiveGasPrice":).*)|((transactionIndex":).*)|((logIndex":).*)"#).unwrap();
...
//     let run_log =
// std::fs::read_to_string("cache/Broadcast.t.sol/31337/run-latest.json").unwrap();     let run_log
...
//     // Firstly run script with non-zero gas prices to ensure that eth_feeHistory contains
// non-zero     // values.
//     cmd.args([
//         "script",
//         "SimpleScript",
...
// {"chain":31337,"estimated_gas_price":"{...}","estimated_total_gas_used":"{...}","
// estimated_amount_required":"{...}","token_symbol":"ETH"} {"chain":"anvil-hardhat","status":"
// success","tx_hash":"0x4f78afe915fceb282c7625a68eb350bc0bf78acb59ad893e5c62b710a37f3156","
// contract_address":null,"block_number":1,"gas_used":"{...}","gas_price":"{...}"} {"status":"
// success","transactions":"[..]/broadcast/Foo.sol/31337/run-latest.json","sensitive":"[..]/cache/
// Foo.sol/31337/run-latest.json"}
...
//     )
//     .unwrap();
//     cmd.forge_fuse().args(["script",
// "script/Counter1.s.sol"]).assert_success().stderr_eq(str![[r#" ...

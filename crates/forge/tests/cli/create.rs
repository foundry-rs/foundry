...
//         config.remappings =
// vec![Remapping::from_str("remapping/=lib/remapping/").unwrap().into()];         config.libraries
// = vec![format!("remapping/MyLib.sol:MyLib:{:?}", Address::random())];     });
...
// fn create_on_chain<F>(info: Option<EnvExternalities>, prj: TestProject, mut cmd: TestCommand, f:
// F) where

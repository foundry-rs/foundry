...
forgetest_init!(symbolic_invariant_frontier_seeding_widens_symbolic_suffix, |prj, cmd| {
    if !z3_available() {
        let _ = sh_eprintln!(
            "skipping symbolic_invariant_frontier_seeding_widens_symbolic_suffix because z3 is not available"
        );
        return;
    }
    ...
    prj.update_config(|config| config.assertions_revert = false);
    prj.add_test(
        "SymbolicInvariantSuffixSeed.t.sol",
        r#"
import "forge-std/Test.sol";

contract SymbolicInvariantSuffixTarget {
    bool public primed;
    bool public failed;
    uint256 public stored;
    ...
    function step(uint256 value) external {
        ...
        if (stored == 123456789 && value == 987654321) failed = true;
    }
}

contract SymbolicInvariantSuffixSeed is Test {
    SymbolicInvariantSuffixTarget target;
...
"#,
    ...
    let frontier_path = find_stateful_frontier_artifact(&prj.root().join("suffix_frontiers"));
    let target_frontier =
        keep_only_matching_frontier(&frontier_path, "second-call stored value", |frontier| {
            frontier["call_index"] == 1
                && frontier["site"]["opcode_name"] == "EQ"
                && frontier["operands"]["result"] == false
        });
    let target_frontier_id = target_frontier["id"].as_u64().unwrap().to_string();
    let sequence_index = target_frontier["sequence_index"].as_u64().unwrap() as usize;
    ...
    // Make the recorded prefix incapable of failing the invariant unless symbolic follow-up can
    // replace both arguments in the two-call suffix.
    let calls = artifact["sequences"][sequence_index].as_array_mut().unwrap();
    for call in &mut calls[..=1] {
        let calldata = call["calldata"].as_str().unwrap();
        call["calldata"] = Value::from(format!("{}{}", &calldata[..10], "0".repeat(64)));
    }
    ...
    let corpus_dir = prj
        .root()
        .join("suffix_corpus")
        .join("SymbolicInvariantSuffixSeed")
        .join("worker0")
        .join("corpus");
    assert!(
        corpus_dir.read_dir().is_ok_and(|mut entries| entries.next().is_some()),
        "stdout={}\nstderr={}",
        output.stdout_lossy(),
        output.stderr_lossy()
    );
    ...
    let replay_output = cmd
        ...
        .clone();
    assert!(
        replay_output.stdout_lossy().contains("invariant_noFailure"),
        "stdout={}\nstderr={}",
        replay_output.stdout_lossy(),
        replay_output.stderr_lossy()
    );
});

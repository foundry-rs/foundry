use crate::integration::utils::{create_test_transaction, wait_for, TestWallet};
...
async fn test_transaction_approval_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn(async move { wallet_server.request_transaction(tx_clone).await });

    // Simulate frontend polling for pending transaction
    wait_for(
        || async {
            wallet.get_pending_transaction().await.map(|opt| opt.is_some()).unwrap_or(false)
        },
...
async fn test_transaction_with_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tx = BrowserTransaction {
        ...
        request: TransactionRequest {
            ...
            input: alloy_rpc_types::TransactionInput::new(Bytes::from(vec![
                0x12, 0x34, 0x56, 0x78,
            ])),
            gas: Some(100000),
            ..Default::default()
    ...
    // Submit transaction
    let wallet_server = wallet.server.clone();
    let handle = tokio::spawn(async move { wallet_server.request_transaction(tx).await });
...
async fn test_transaction_timeout() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn(async move {
        ...
        tokio::time::timeout(Duration::from_secs(2), wallet_server.request_transaction(tx)).await
    });

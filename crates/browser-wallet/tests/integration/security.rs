use crate::integration::utils::{create_test_signing_request, create_test_transaction, TestWallet};
...
async fn test_transaction_replay_prevention() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn({
        ...
        async move { wallet_server.request_transaction(tx_clone).await }
    });
    ...
    let replay_handle = tokio::spawn({
        let wallet_server = wallet.server.clone();
        async move { wallet_server.request_transaction(tx).await }
    });
...
async fn test_signing_replay_prevention() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn({
        ...
        async move { wallet_server.request_signing(req_clone).await }
    });
    ...
    let replay_handle = tokio::spawn({
        let wallet_server = wallet.server.clone();
        async move { wallet_server.request_signing(request).await }
    });
...
async fn test_concurrent_transaction_safety() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let polling_handle = tokio::spawn(async move {
        for _ in 0..10 {
            ...
            let response = client.get(format!("{base_url}/api/transaction/pending")).send().await;
...
async fn test_malformed_transaction_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut tx =
        BrowserTransaction { id: "malformed-test".to_string(), request: Default::default() };

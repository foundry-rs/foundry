...
impl<N: Network> BrowserWalletState<N> {
    ...
    pub(crate) fn new(session_token: String, development: bool) -> Self {
        Self {
    ...
    pub(crate) async fn get_connection(&self) -> Option<Connection> {
        *self.connection.read().await
    }

    /// Set connection information.
    pub(crate) async fn set_connection(&self, connection: Option<Connection>) {
        *self.connection.write().await = connection;
    }

    /// Add a transaction request.
    pub(crate) async fn add_transaction_request(&self, request: BrowserTransactionRequest<N>) {
        self.transactions.lock().await.add_request(request);
    }

    /// Check if a transaction request exists.
    pub(crate) async fn has_transaction_request(&self, id: &Uuid) -> bool {
        self.transactions.lock().await.has_request(id)
    }

    /// Read the next transaction request.
    pub(crate) async fn read_next_transaction_request(
        &self,
    ) -> Option<BrowserTransactionRequest<N>> {
        self.transactions.lock().await.read_request().cloned()
    }

    // Remove a transaction request.
    pub(crate) async fn remove_transaction_request(&self, id: &Uuid) {
        self.transactions.lock().await.remove_request(id);
    }

    /// Add transaction response.
    pub(crate) async fn add_transaction_response(&self, response: BrowserTransactionResponse) {
        let id = response.id;
    ...
    pub(crate) async fn get_transaction_response(
        &self,
        id: &Uuid,
    ) -> Option<BrowserTransactionResponse> {
        self.transactions.lock().await.get_response(id)
    }

    /// Add a signing request.
    pub(crate) async fn add_signing_request(&self, request: BrowserSignRequest) {
        self.signings.lock().await.add_request(request);
    }

    /// Check if a signing request exists.
    pub(crate) async fn has_signing_request(&self, id: &Uuid) -> bool {
        self.signings.lock().await.has_request(id)
    }

    /// Read the next signing request.
    pub(crate) async fn read_next_signing_request(&self) -> Option<BrowserSignRequest> {
        self.signings.lock().await.read_request().cloned()
    }

    /// Remove a signing request.
    pub(crate) async fn remove_signing_request(&self, id: &Uuid) {
        self.signings.lock().await.remove_request(id);
    }

    /// Add signing response.
    pub(crate) async fn add_signing_response(&self, response: BrowserSignResponse) {
        let id = response.id;
    ...
    pub(crate) async fn get_signing_response(&self, id: &Uuid) -> Option<BrowserSignResponse> {

...
impl<Req, Res> RequestQueue<Req, Res> {
    ...
    pub(super) fn add_request(&mut self, request: Req) {
        self.requests.push_back(request);
    }

    /// Check if the queue contains any pending requests matching the given ID.
    pub(super) fn has_request(&self, id: &Uuid) -> bool
    where
        Req: HasId,
    {
    ...
    pub(super) fn read_request(&self) -> Option<&Req> {
        self.requests.front()
    }

    /// Remove a request by its ID.
    pub(super) fn remove_request(&mut self, id: &Uuid) -> Option<Req>
    where
        Req: HasId,
    {
    ...
    pub(super) fn add_response(&mut self, id: Uuid, response: Res) {
        self.responses.insert(id, response);
    }

    /// Get a response by its ID, removing it from the queue.
    pub(super) fn get_response(&mut self, id: &Uuid) -> Option<Res> {

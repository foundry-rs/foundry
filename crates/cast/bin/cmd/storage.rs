...
//             eyre::bail!("You must provide an Etherscan API key if you're fetching a remote
// contract's storage.");         }
...
//                 eprintln!("The requested contract was compiled with {version} while the minimum
// version for storage layouts is {MIN_SOLC} and as a result the output may be empty.");            
// let solc = Solc::find_or_install_svm_version(MIN_SOLC.to_string())?;                 project.solc
// = solc;                 project.auto_detect = false;
...
//     pretty: bool,
// ) -> Result<()> { if is_storage_layout_empty(&artifact.storage_layout) { eprintln!("Storage
//   layout is empty."); Ok(()) } else { let layout =
//   artifact.storage_layout.as_ref().unwrap().clone(); let values = fetch_storage_slots(provider,
//   address, &layout).await?; print_storage(layout, values, pretty) }
// }
...
// ) -> Result<Vec<B256>> { TODO: Batch request let futures: Vec<_> = layout .storage .iter()
//   .map(|slot| { let slot = B256::from(U256::from_str(&slot.slot)?);
...
//     let mut table = Table::new();
//     table.load_preset(ASCII_MARKDOWN);
//     // Added a new column "Hex Value"
//     table.set_header(["Name", "Type", "Slot", "Offset", "Bytes", "Value", "Hex Value",
// "Contract"]);
...
//         // Convert the raw value bytes to a hexadecimal string
...
//             &storage_type.map_or("?", |t| &t.number_of_bytes),
//             &converted_value.to_string(),
//             &hex_value, // Added hex value here
//             &slot.contract,
//         ]);
//     }

...
impl DocBuilder {
    ...
    fn write_summary_section(
        ...
        let grouped = grouped.into_iter().sorted_by(|(lhs, _), (rhs, _)| {
            let lhs_at_end = lhs.extension().is_some_and(|ext| ext == Self::SOL_EXT);
            let rhs_at_end = rhs.extension().is_some_and(|ext| ext == Self::SOL_EXT);
        ...
        for (path, files) in grouped {
            if path.extension().is_some_and(|ext| ext == Self::SOL_EXT) {
                for file in files {

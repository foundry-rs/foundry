...
pub(crate) fn max_u256_expr(left: SymExpr, right: SymExpr) -> SymExpr {
...
impl SymMemory {
    ...
    pub(crate) fn store_symbolic_bytes(&mut self, offset: SymExpr, bytes: Vec<SymWord>) {
        if bytes.is_empty() {
            return;
        }
        self.epoch = self.epoch.saturating_add(1);

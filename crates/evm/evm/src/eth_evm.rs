...
///
/// This is the Eth-specific inspector bound used by [`EthEvm`] and [`EthHandler`].
/// For network-agnostic code, use `FoundryInspectorExt` instead.
...
pub fn new_eth_evm<'db, I: EthInspectorExt>(
...
pub struct EthEvm<'db, I: EthInspectorExt> {
...
impl<'db, I: EthInspectorExt> EthEvm<'db, I> {
    ...
    pub fn run_execution(
        ...
        let mut handler = EthHandler::<I>::default();
...
impl<'db, I: EthInspectorExt> Evm for EthEvm<'db, I> {
    ...
    fn set_inspector_enabled(&mut self, _enabled: bool) {
        unimplemented!("EthEvm is always inspecting")
    }
...
impl<'db, I: EthInspectorExt> Deref for EthEvm<'db, I> {
...
impl<I: EthInspectorExt> DerefMut for EthEvm<'_, I> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.inner.ctx
    }
}

impl<I: EthInspectorExt> NestedEvm for EthEvm<'_, I> {
    fn journal_inner_mut(&mut self) -> &mut JournaledState {
        &mut self.inner.ctx.journaled_state.inner
    }

    fn run_execution(&mut self, frame: FrameInput) -> Result<FrameResult, EVMError<DatabaseError>> {
        EthEvm::run_execution(self, frame)
    }
    ...
    fn take_journal_inner(&mut self) -> JournaledState {
        std::mem::take(&mut self.inner.ctx.journaled_state.inner)
    }
}

pub struct EthHandler<'db, I: EthInspectorExt> {
    create2_overrides: Vec<(usize, CallInputs)>,
    _phantom: PhantomData<(&'db mut dyn DatabaseExt, I)>,
}

impl<I: EthInspectorExt> Default for EthHandler<'_, I> {
    fn default() -> Self {
        Self { create2_overrides: Vec::new(), _phantom: PhantomData }
    }
}

// Blanket Handler implementation for EthHandler, needed for implementing the InspectorHandler
// trait.
impl<'db, I: EthInspectorExt> Handler for EthHandler<'db, I> {
    type Evm = RevmEvm<
        EthEvmContext<&'db mut dyn DatabaseExt>,
        I,
        EthInstructions<EthInterpreter, EthEvmContext<&'db mut dyn DatabaseExt>>,
...
impl<'db, I: EthInspectorExt> EthHandler<'db, I> {
...
impl<I: EthInspectorExt> InspectorHandler for EthHandler<'_, I> {

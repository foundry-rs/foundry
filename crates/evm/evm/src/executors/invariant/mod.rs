        ...
        let _ = self.runner.run(&invariant_strategy, |first_input| {
            ...
            while current_run.depth < self.config.depth {
                ...
                if discarded {
                    ...
                    // This step updates the state dictionary and therefore invalidates the ValueTree in use by the current run.
                    // This manifests itself in proptest observing a different input case than what it was called with, 
                    // and creates inconsistencies whenever proptest tries to use the input case after test execution.

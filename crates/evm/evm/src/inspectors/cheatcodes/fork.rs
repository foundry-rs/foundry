use super::{ext::value_to_token, fmt_err, Cheatcodes, Error, Result};
...
pub fn apply<DB: DatabaseExt>(
    ...
    let result = match call {
        ...
        HEVMCalls::Transact0(inner) => data
            .db
            .transact(None, inner.0.into(), data.env, &mut data.journaled_state, state)
            .map(empty)
            .map_err(Into::into),
...
fn eth_getlogs<DB: DatabaseExt>(data: &EVMData<'_, DB>, inner: &EthGetLogsCall) -> Result {
...
fn rpc<DB: DatabaseExt>(data: &EVMData<'_, DB>, inner: &RpcCall) -> Result {
    let url = data.db.active_fork_url().ok_or(fmt_err!("No active fork url found"))?;
    let provider = ProviderBuilder::new(url).build()?;

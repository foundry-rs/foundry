...
macro_rules! fmt_err {
    ($msg:literal $(,)?) => {
        $crate::inspectors::cheatcodes::Error::fmt(::std::format_args!($msg))
    };
    ($err:expr $(,)?) => {
        <$crate::inspectors::cheatcodes::Error as ::std::convert::From<_>>::from($err)
    };
    ($fmt:expr, $($arg:tt)*) => {
        $crate::inspectors::cheatcodes::Error::fmt(::std::format_args!($fmt, $($arg)*))
    };
}

macro_rules! bail {
    ($msg:literal $(,)?) => {
        return ::std::result::Result::Err($crate::inspectors::cheatcodes::fmt_err!($msg))
    };
    ($err:expr $(,)?) => {
        return ::std::result::Result::Err($crate::inspectors::cheatcodes::fmt_err!($err))
    };
    ($fmt:expr, $($arg:tt)*) => {
        return ::std::result::Result::Err($crate::inspectors::cheatcodes::fmt_err!($fmt, $($arg)*))
    };
}
...
macro_rules! ensure {
    ($cond:expr $(,)?) => {
        if !$cond {
            return ::std::result::Result::Err($crate::inspectors::cheatcodes::Error::custom(

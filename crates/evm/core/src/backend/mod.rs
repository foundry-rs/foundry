...
impl<FEN: FoundryEvmNetwork> Backend<FEN> {
    ...
    fn get_block_number_and_block_for_transaction(
        ...
        if let Some(tx_block) = tx.block_number() {
            ...
            Ok(TransactionForkTarget { fork_block, transaction: tx, block, mined: true })
        } else {
            ...
            Ok(TransactionForkTarget { fork_block, transaction: tx, block, mined: false })
        }
    }
    ...
    fn roll_fork_to_transaction_with_context(
        ...
        let id = self.ensure_fork(id)?;
        let affects_active = self.is_active_fork(id);
        let TransactionForkTarget { fork_block, block, .. } =
        ...
        update_env_block(&mut staged_evm_env, block.header());
    ...
    fn roll_fork_to_transaction_inner(
        ...
        let TransactionForkTarget { fork_block, block, mined, .. } =
            self.get_block_number_and_block_for_transaction(id, transaction)?;
        ...
        if FEN::EvmFactory::NEEDS_BLOCK_CONTEXT && transaction_index.is_none() {
            eyre::bail!(
                "transaction {transaction:?} is missing from block {}",
                block.header().number()
            );
    ...
    fn replay_until(
        ...
        eyre::ensure!(
            !FEN::EvmFactory::NEEDS_BLOCK_CONTEXT || block_context.is_some(),
...
impl<FEN: FoundryEvmNetwork> DatabaseExt<FEN::EvmFactory> for Backend<FEN> {
    ...
    fn transact(
        ...
        let TransactionForkTarget { transaction: tx, block, .. } =
            self.get_block_number_and_block_for_transaction(id, transaction)?;
        let tx_env = TxEnvFor::<FEN>::from_any_rpc_transaction(&tx)?;
...
mod tests {
    ...
    async fn temporary_backend_preserves_fork_position() {
        ...
        for position in [
            ...
            let temporary =
                Backend::<EthEvmNetwork>::new_with_fork(&fork_id, fork, journaled_state).unwrap();
    ...
    async fn fork_factory_boundary_preserves_explicit_execution_overrides() {
        ...
        fn target_fork(opts: EvmOpts, url: String) -> CreateFork {
            CreateFork { url, enable_caching: false, evm_opts: opts, resolved: None }
        }

...
impl Cheatcode for mockCall_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        ccx.ecx.inner.inner.journaled_state.load_account(*callee)?;
...
fn make_acc_non_empty(callee: &Address, ccx: &mut CheatsCtxt) -> Result {
    let acc = ccx.ecx.load_account_code(*callee);

    let empty_bytecode = acc.as_ref().map_or(true, |code| code.data.is_empty());

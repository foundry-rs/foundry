...
use foundry_evm_core::{fork::CreateFork, Env};
...
impl Cheatcode for rollFork_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        ccx.ecx.inner.inner.db().roll_fork(
            None,
            (*blockNumber).to(),
            &mut env,
            &mut journaled_state,
...
impl Cheatcode for rollFork_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        let mut journaled_state = ccx.ecx.inner.inner.journaled_state.clone();
        ccx.ecx.inner.inner.db().roll_fork_to_transaction(
            None,
            *txHash,
            &mut env,
            &mut journaled_state,
...
impl Cheatcode for selectForkCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        let mut journaled_state = ccx.ecx.inner.inner.journaled_state.clone();
        ccx.ecx.inner.inner.db().select_fork(*forkId, &mut env, &mut journaled_state)?;
        Ok(Default::default())
    }
}
...
impl Cheatcode for allowCheatcodesCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        ccx.ecx.inner.inner.db().allow_cheatcode_access(*account);
        Ok(Default::default())
    }
}
...
impl Cheatcode for makePersistent_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        let Self { account } = self;
        ccx.ecx.inner.inner.db().add_persistent_account(*account);
        Ok(Default::default())
    }
}
...
impl Cheatcode for makePersistent_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        ccx.ecx.inner.inner.db().add_persistent_account(*account0);
        ccx.ecx.inner.inner.db().add_persistent_account(*account1);
        Ok(Default::default())
    }
}
...
impl Cheatcode for makePersistent_2Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        ccx.ecx.inner.inner.db().add_persistent_account(*account0);
        ccx.ecx.inner.inner.db().add_persistent_account(*account1);
        ccx.ecx.inner.inner.db().add_persistent_account(*account2);
        Ok(Default::default())
    }
}
...
impl Cheatcode for revokePersistent_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        let Self { account } = self;
        ccx.ecx.inner.inner.db().remove_persistent_account(account);
        Ok(Default::default())
    }
}
...
impl Cheatcode for isPersistentCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        let Self { account } = self;
        Ok(ccx.ecx.inner.inner.db().is_persistent(account).abi_encode())
    }
}
...
fn create_select_fork(ccx: &mut CheatsCtxt, url_or_alias: &str, block: Option<u64>) -> Result {
    ...
    let mut journaled_state = ccx.ecx.inner.inner.journaled_state.clone();
    let id = ccx.ecx.inner.inner.db().create_select_fork(fork, &mut env, &mut journaled_state)?;
    Ok(id.abi_encode())
}
...
fn create_fork(ccx: &mut CheatsCtxt, url_or_alias: &str, block: Option<u64>) -> Result {
    let fork = create_fork_request(ccx, url_or_alias, block)?;
    let id = ccx.ecx.inner.inner.db().create_fork(fork)?;
    Ok(id.abi_encode())
}
...
fn create_select_fork_at_transaction(
    ...
    let mut journaled_state = ccx.ecx.inner.inner.journaled_state.clone();
    let id = ccx.ecx.inner.inner.db().create_select_fork_at_transaction(
        fork,
        &mut env,
        &mut journaled_state,
        *transaction,
...
fn create_fork_at_transaction(
    ...
    let fork = create_fork_request(ccx, url_or_alias, None)?;
    let id = ccx.ecx.inner.inner.db().create_fork_at_transaction(fork, *transaction)?;
    Ok(id.abi_encode())
}
...
fn transact(
    ...
    let mut journaled_state = ccx.ecx.inner.inner.journaled_state.clone();
    ccx.ecx.inner.inner.db().transact(
        fork_id,
        transaction,
        env,
        &mut journaled_state,
...
fn persist_caller(ccx: &mut CheatsCtxt) {
    ccx.ecx.inner.inner.db().add_persistent_account(ccx.caller);
}

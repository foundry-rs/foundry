...
// The expected `stack_input` here is already handled by the `get_stack_inputs_for_opcode()`
// function.
...
pub(crate) fn get_stack_inputs_for_opcode(opcode: u8, stack: Vec<U256>) -> Vec<U256> {
    match opcode {
        ...
        opcode::CALL => (0..7).map(|i| peak_stack(&stack, i)).collect::<Vec<U256>>(),
        opcode::CALLCODE => (0..7).map(|i| peak_stack(&stack, i)).collect::<Vec<U256>>(),
        opcode::RETURN => {
            let a = peak_stack(&stack, 0);
            let b = peak_stack(&stack, 1);
            vec![a, b]
        }
        opcode::DELEGATECALL => (0..6).map(|i| peak_stack(&stack, i)).collect::<Vec<U256>>(),
        opcode::CREATE2 => {
            let a = peak_stack(&stack, 0);
            let b = peak_stack(&stack, 1);
            let c = peak_stack(&stack, 2);
        ...
        opcode::STATICCALL => (0..6).map(|i| peak_stack(&stack, i)).collect::<Vec<U256>>(),
        opcode::REVERT => {
            let a = peak_stack(&stack, 0);
            let b = peak_stack(&stack, 1);
            vec![a, b]

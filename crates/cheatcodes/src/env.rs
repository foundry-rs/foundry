...
impl Cheatcode for envUint_0Call {
    fn apply(&self, _state: &mut Cheatcodes) -> Result {
        let Self { name } = self;
        env_uint(name).map_err(|e| fmt_err!("Could not parse U256 environment variable {name}. Make sure it's a valid uint, and ideally 0x-prefixed if its using hex: {e}"))
    }
}
...
fn env_uint(key: &str) -> Result {
    match get_env(key) {
        Ok(val) => Ok(string::parse::<U256>(&val).unwrap_or(

...
impl Cheatcode for copyStorageCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt) -> Result {
        ...
        if let Ok(from_account) = ccx.ecx.inner.inner.journaled_state.load_account(*from) {
            let from_storage = from_account.storage.clone();
            if let Ok(mut to_account) = ccx.ecx.inner.inner.journaled_state.load_account(*to) {

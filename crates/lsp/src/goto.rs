...
pub fn goto_references(
    ...
    for ref_id in refs {
        if let Some(transitive_refs) = all_refs.get(ref_id) {
            results.extend(transitive_refs.iter().cloned());
        }
    }
...
mod tests {
    ...
    fn test_goto_declaration_basic() {
        ...
        // The declaration should be on the function parameter line (line 20, 0-based = 19)
        // But due to AST structure, we just verify we get a reasonable result

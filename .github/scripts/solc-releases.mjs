...
function compose({ linux, linuxNative, linuxLegacy, linuxOld, mac, macNative, windows }) {
  ...
  return {
    ...
    'macosx-amd64': mac,
    'macosx-aarch64': merge(
      retain(mac, version => compare(version, '0.8.5') < 0 || compare(version, '0.8.24') > 0),
      macNative,
    ),
...
export async function writeReleases(bundle) {
  // The versioned path deliberately fails on an svm-rs upgrade so its platform
  // rules and this acquisition step must be reviewed together.
  const svm = join(bundle, 'cargo-home/vendor', 'svm-rs-0.5.27');
  ...
  const [linux, linuxNative, linuxLegacy, mac, macNative, windows] = await Promise.all([
    json(`${solc}/linux-amd64/list.json`),
    json(`${solc}/linux-arm64/list.json`),
    json(linuxArm),
    json(`${solc}/macosx-amd64/list.json`),
    json(macArm),
    json(`${solc}/windows-amd64/list.json`),
  ]);
  const directory = join(bundle, 'solc');
  mkdirSync(directory);
  for (const [platform, list] of Object.entries(
    compose({ linux, linuxNative, linuxLegacy, linuxOld, mac, macNative, windows }),
  )) {

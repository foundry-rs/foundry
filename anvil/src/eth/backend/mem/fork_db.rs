...
impl Database for ForkedDatabase {
    ...
    fn storage(&mut self, address: Address, index: U256) -> U256 {
        tokio::task::block_in_place(|| Database::storage(&mut self.cache_db, address, index))
    }

    fn block_hash(&mut self, number: U256) -> H256 {
        tokio::task::block_in_place(|| self.cache_db.block_hash(number))
    }
}
...
impl DatabaseRef for ForkedDatabase {
    ...
    fn storage(&self, address: Address, index: U256) -> U256 {
        tokio::task::block_in_place(|| DatabaseRef::storage(&self.cache_db, address, index))
    }
